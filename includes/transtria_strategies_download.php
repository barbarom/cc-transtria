...
$objPHPExcel->getProperties()
    ...
    ->setTitle("Strategies by Study ID");
...
$objPHPExcel->setActiveSheetIndex(0);
	...
	$column_names = array( "Study ID", "Result ID", "Result Descr", "Study Grouping ID" );
...
//TODO, use wp->prepare
	$question_sql = 
		"
		SELECT DISTINCT `wp_transtria_code_results`.`ID`, `wp_transtria_code_results`.`result`, `wp_transtria_codetbl`.`descr`, 
		`wp_transtria_studies`.`StudyGroupingID` 
...
for ($i = 0; $i < $column_num; $i++){
    ...
    $objPHPExcel->getActiveSheet()->setCellValue( $column.$rowCount, $column_names[ $i] );
    $column++;
}
