...
class CC_Transtria_Extras {
	...
	public function enqueue_scripts() {
		if ( cc_transtria_is_component() ) {
			...
			if( cc_transtria_on_assignments_screen() ){
				wp_enqueue_script( $this->plugin_slug . 'transtria_revamp_assignments_js', plugins_url( 'js/transtria_revamp_assignments.js', __FILE__ ), array( 'jquery' ), '1.6' );
			}
			
			if( cc_transtria_on_analysis_screen() ){
				wp_enqueue_script( $this->plugin_slug . 'transtria_revamp_analysis_js', plugins_url( 'js/transtria_revamp_analysis.js', __FILE__ ), array( 'jquery' ), '1.6' );
			}
			...
			wp_localize_script( 
				...
				array( 
					...
					'stage_lookup' => array(
						array( "value" => 1, "descr" => "Adoption"),
						array( "value" => 2, "descr" => "Implementation"),
						array( "value" => 3, "descr" => "Enforcement/ maintenance"),
						array( "value" => 999, "descr" => "Insufficient information"),
						),
	...
	*/
	...
	public function run_intermediate_analysis(){
		...
		echo json_encode( $intermediate_calcs );
