 ...
 * PHPExcel
 *
 * Transtria: meta table xls builder
 *
 */
...
$objPHPExcel->getProperties()
    ...
    ->setTitle("Metadata table data: raw");
	
	
$objPHPExcel->setActiveSheetIndex(0);

	//number of columns
	$column_num_seq = "SELECT COUNT(`COLUMN_NAME`) FROM `INFORMATION_SCHEMA`.`COLUMNS` WHERE `TABLE_SCHEMA`='" . $schema . "' AND `TABLE_NAME`='wp_transtria_metadata'";
	$column_name_seq = "SELECT `COLUMN_NAME` FROM `INFORMATION_SCHEMA`.`COLUMNS` WHERE `TABLE_SCHEMA`='" . $schema . "' AND `TABLE_NAME`='wp_transtria_metadata'";
...
$objWriter->save( $root_ish . '/PHPExcel/transtria/metadata.xls');
